...
class MarkDistinctTest : public OperatorTestBase {
 protected:
  ...
  std::function<VectorPtr()> baseSupplier() {
    return [&]() {
  ...
  void markDistinctTest(
    ...
    auto field1 =
        std::make_shared<core::FieldAccessTypedExpr>(exprTypeSupplier(), "c0");
...
TEST_F(MarkDistinctTest, basicTinyIntTest) {
  ...
  markDistinctTest<cppColType>(baseSupplier<cppColType>());
}
